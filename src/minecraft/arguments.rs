...
pub fn generate_data_map(
  ...
  let mut map = HashMap::from([
    ...
    ("natives_dir", path.join("versions").join(config.client.version).join("natives").to_str().unwrap().to_string()),
    ...
    ("version_jar", path.join("versions").join(config.client.version).join("client.jar").to_str().unwrap().to_string())
  ]);
